...
async function errorPluginCallback(fastify: FastifyInstance): Promise<void> {
  fastify.setErrorHandler(function (error: FastifyError, _request, reply) {
    const prismaError = error as FastifyError & { code?: string; meta?: { target?: unknown } };
    ...
    if (
      ...
      prismaError.meta?.target.includes('number')
    ) {
      reply.code(409).send({ error: 'License number already exists' });
      return;
    }
