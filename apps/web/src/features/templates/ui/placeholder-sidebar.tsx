  ...
  if (mappings.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Click a cell to assign a placeholder</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-medium">Assigned ({mappings.length})</p>
      </div>
      {sorted.map((mapping) => (
        <div
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="truncate text-xs font-mono">{mapping.value}</span>
          </div>
          <Button
            variant="ghost"
            size="icon"
