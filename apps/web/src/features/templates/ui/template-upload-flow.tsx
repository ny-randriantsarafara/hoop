...
import { previewTemplate, uploadTemplate } from '../api/template-api';
...
export function TemplateUploadFlow() {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      if (ext !== 'xlsx') {
        toast({
          title: 'Only .xlsx files are supported for visual editing',
          variant: 'destructive',
        });
        return;
      }
  ...
  function handleCellClick(row: number, col: number) {
    ...
    const top = placement === 'below' ? rect.bottom + gap : rect.top - dropdownHeight - gap;
  ...
  function handlePlaceholderSelect(_key: string, updatedValue: string) {
    ...
    setCellMappings((prev) => {
      const filtered = prev.filter((m) => m.row !== selectedCell.row || m.col !== selectedCell.col);
  ...
  if (state.step === 'editor') {
    const currentValue = selectedCell
      ? (cellMappings.find((m) => m.row === selectedCell.row && m.col === selectedCell.col)
          ?.value ?? '')
      : '';
