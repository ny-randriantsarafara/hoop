...
import { playerFilterFields } from './player-filter-fields';
import { licenseFilterFields } from './license-filter-fields';
import type { FilterFieldDefinition } from './filter-field-definition';
  ...
  it('enum fields have at least one option', () => {
    for (const field of fields) {
      if (field.type === 'enum') {
        ...
        if (!field.options) {
          throw new Error('Expected enum options to be defined');
        }
...
describe('licenseFilterFields', () => {
  ...
  it('status field has active and expired options', () => {
    ...
    if (!statusField?.options) {
      throw new Error('Expected status field options to be defined');
    }
